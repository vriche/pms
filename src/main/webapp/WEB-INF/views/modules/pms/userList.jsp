			...
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出用户数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						...
						$("#searchForm").submit();
					    $("#searchForm").attr("action","${ctx}/pms/user/list");
					}
				},{buttonsFocus:1});
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="pms:user:edit"><li><a href="${ctx}/pms/user/form">业主添加</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/pms/user/" method="post" class="breadcrumb form-search">
		...
		<div>
			<label>归属公司：</label>
			<tags:treeselect id="company" name="proCompanyId" value="${user.company.id}" labelName="company.name" labelValue="${user.company.name}" 
		...
		<!-- input id="proCompanyId" name="proCompanyId" type="hidden" value=""/ -->
		...
		<div style="margin-top:8px;">
			...
			<tags:treeselect id="houseIds" 
				name="houseIds" 
				value="${user.house.id}" 
				labelName="house.name"
				labelValue="${user.house.name}" 
				title="房产"
