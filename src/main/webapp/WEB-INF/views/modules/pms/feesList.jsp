...
<body>
	...
	<form:form id="searchForm" modelAttribute="fees" action="${ctx}/pms/fees/" method="post" class="breadcrumb form-search">
		...
		<form:select id="proCompanyId" name="proCompanyId" path="company.id"  class="text medium;required">
					<form:options items="${proCompanyList}" itemLabel="name" itemValue="id" htmlEscape="false" />
		</form:select>
		
	    <label>收费类型:</label>
		<form:select  id="feesType" name="feesType" path="feesType"  class="input-small text medium;required">
					<form:options items="${fns:getDictList('pms_fees_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		</form:select>
			

		<label>费项编号 ：</label><form:input path="code" htmlEscape="false" maxlength="50" class="input-small"/>
		...
		<c:forEach items="${page.list}" var="fees">
			<tr>
				...
				<shiro:hasPermission name="pms:fees:edit"></shiro:hasPermission><td>
					...
					<a href="${ctx}/pms/fees/delete?id=${fees.id}" onclick="return confirmx('确认要删除该收费项目吗？', this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
