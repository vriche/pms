...
function dhtmlXGridObject(id){
 ...
 this._key_events ={k13_1_0: function(){var rowInd = this.rowsCol._dhx_find(this.row)
...
//	    if(this.rowsBuffer == '') this.rowsBuffer =new Array(new dhtmlxArray(0),new dhtmlxArray(0));
...
this.setAllChecks = function(cInd,v){this.forEachRow(function(id){if(this.cells(id,cInd).isCheckbox())this.cells(id,cInd).setValue(v)})}; 
 ...
 if(ev.button==0){
	 if((this.onLCL)&&(!this.onLCL(el.parentNode.idd,el._cellIndex,ev)))return;
 }
 if(ev.button==2){
 ...
 if(this._ecspn)
{
 ...
 for(var i=0;i<this.rowsCol.length;i++){
 	 if(this.rowsCol[i]){
	 	 ...
	 	 if(cell.isCheckbox()){
			 var v = cell.getValue();
			 if(v!="0" && v!="") d[d.length]=this.rowsCol[i].idd;
	 	 }
 	 }
...
if (!this.obj.rows[0])return;var tb = this.obj.rows[0].parentNode;var tr = tb.removeChild(tb.childNodes[0], true)
 ...
 dhtmlXGridObject.prototype.render_row = function(ind){if (!this.rowsBuffer[ind])return -1;if (this.rowsBuffer[ind]._parser){var r = this.rowsBuffer[ind];if (this.rowsAr[r.idd] && this.rowsAr[r.idd].tagName=="TR")return this.rowsBuffer[ind]=this.rowsAr[r.idd];var row = this._prepareRow(r.idd);this.rowsBuffer[ind]=row;this.rowsAr[r.idd]=row;r._parser.call(this, row, r.data);this._postRowProcessing(row);return row};return this.rowsBuffer[ind]};
...
dhtmlXGridObject.prototype._in_header_master_checkbox= function(tag,index,data){            // the name contains "_in_header_"+shortcut_name
